...
- **Speculative decoding survives concurrency now.** [EAGLE-3](https://arxiv.org/abs/2503.01840) drafts from fused multi-layer hidden states (not just the final layer), raising acceptance rates; EAGLE 3.1 (May 2026, with the vLLM team) reports **2.03× at concurrency 1, 1.71× at C=4, and still 1.66× at C=16**. The old "spec decode is a single-stream trick" heuristic is dead; it now earns its place in moderately-batched interactive tiers, and only the saturated batch tier turns it off.
...
- **Disaggregation is mainstream, not exotic:** prefill/decode separation shipped as a first-class vLLM abstraction (KVConnector), with Dynamo/Mooncake-class orchestration above it — my [KV-cache doc](/llm-sysdesign/2kvcache/) covers that frontier in depth; this design's §4 diagram is now the default shape at scale, not the ambitious one.
