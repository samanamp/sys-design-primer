...
**Now sanity-check the batch against the communication roofline**, using the framework from the [JAX scaling book](https://jax-ml.github.io/scaling-book/training/). The controlling constant is the **ICI arithmetic intensity** — per-chip compute rate over per-link ICI bandwidth:

$$
\alpha = \frac{C}{W_{ici}} = \frac{4.59 \times 10^{14}}{2 \times 9 \times 10^{10}} \approx 2{,}550 \text{ tokens}
$$

Data parallelism and FSDP stay **compute-bound** (collectives fully hideable) only while per-chip batch exceeds $\alpha / M_X$, where $M_X$ is the number of torus axes carrying the sharding — more axes, proportionally more aggregate link bandwidth. Our layout puts FSDP on fragments of two axes ($M_X \approx 2$), so the floor is **~1,275 tokens/chip**. The chosen batch gives **2,048 tokens/chip — compute-bound with a ~1.6× margin**, but not a large one: this is the roofline that says the ~37M-token global batch is near the *bottom* of what this mesh can run efficiently, not an arbitrary round number. Shrinking global batch much further (e.g. for optimizer reasons) means either shrinking the FSDP degree, adding TP (the mixed FSDP+TP floor is $B/N \geq \alpha^2 / (M_X M_Y F)$ per the book), or accepting exposed all-gathers.
...
- **FSDP-style parameter/optimizer sharding on the remaining 35-extent** (5-of-Y × 7-of-Z). ZeRO-3 semantics via GSPMD: all-gather weights around each layer's compute, reduce-scatter grads — prefetched and overlapped. Do the volume math honestly: each chip's forward touches the shared/attention weights plus its 1/64 slice of experts — **~1T/64 ≈ 15.6B params ≈ 31 GB in bf16 of *used* weights per chip**. Those 31 GB cannot stay resident between forward and backward (the memory bar above has ~30 GB of total slack, not 31 GB for weights alone), so the schedule is **per-layer gather → use → discard, in both passes**: ~31 GB of all-gather in forward, ~31 GB again in backward, plus ~31 GB of grad reduce-scatter ≈ **~90 GB/chip/step**. That sounds enormous until you divide by the step: 90 GB / 5.3 s ≈ **17 GB/s sustained** against ~180 GB/s of bidirectional bandwidth per ICI link (9×10¹⁰ B/s each way, scaling-book numbers) — and the roofline check above says the same thing in closed form: 2,048 tokens/chip > 2,550/M_X, so the all-gathers hide. The design question this raises — *which* layers, if any, keep gathered weights resident fwd→bwd — is answered by the memory bar: almost none; you pay the double gather and it's still cheap.
- **No tensor-parallel matmul sharding.** The memory math didn't demand it, and the scaling-book roofline makes the negative case quantitative: TP over $Y$ chips goes **communication-bound once $Y > M_Y \cdot F / 2{,}550$**, where $F$ is the sharded feed-forward dimension. For a dense d_ff ≈ 4·d_model ≈ 64K that allows Y up to ~25·M_Y — but the MoE's *fine-grained experts* have small per-expert F (order 2–4K), so TP inside expert FFNs is comms-bound almost immediately (Y ≈ 1–2 on a single axis). The architecture itself rules TP out where most of the FLOPs live. Beyond that, TP adds per-layer latency-sensitive collectives that compete with EP and CP for the fast axes. Add TP ≤ 4 only if per-layer weight size or activation memory forces it. Precision matters here: the claim is *no TP* — not "no sequence-dimension sharding at all"; CP is doing that job, and saying so is the stronger signal.
...
- **Data parallelism across the DCN boundary.** The scaling book's DCN roofline: with ~6.25 GB/s of DCN per chip, the DCN arithmetic intensity is $C/W_{dcn} \approx 73{,}440$ — DP over DCN stays compute-bound as long as **per-pod batch exceeds ~73K tokens**. Ours is ~18.4M tokens/pod, a **~250× margin**, which is why cross-pod DP is the one strategy that survives the 100×-slower link. The two pods are pure DP replicas; the only cross-pod traffic is gradient reduction, overlapped with the backward pass. This is exactly the multislice pattern Google has demonstrated publicly at 50K+ chips on v5e, and Trillium's launch materials report ~96–99% scaling efficiency at 12 pods depending on configuration (Google blog) for DP-over-DCN workloads — evidence the boundary placement is right.
...
Adding six more pods as DP replicas is mechanically easy under multislice — that's the point of putting DP on DCN. What breaks is upstream of topology: global batch grows 4× (~147M tokens), which is well past the useful batch-size regime — so I'd reject naive batch growth and instead shrink per-pod batch (fewer sequences per (fsdp, expert) coordinate, or deeper CP), trading MFU for optimizer sanity, with LR/warmup retuned either way. The roofline from section 2 prices that trade: per-chip batch is already at 2,048 tokens against a ~1,275-token FSDP floor (2,550/M_X), so halving per-pod batch drops below it — the fix is widening the FSDP sharding onto more axis extent (raising M_X) or accepting partially exposed all-gathers, and saying which you'd pick is the staff move. DCN is not the problem: even at a quarter of today's per-pod batch you're ~60× above the ~73K-token DCN floor. DCN gradient reduction volume per pod is unchanged (all-reduce cost per replica doesn't grow with replica count in the bandwidth term), but scheduling goodput drops — acquiring and holding 8 aligned pods is a real availability negotiation. And failure frequency scales linearly with chips, so the reliability machinery of section 5 goes from "important" to "the job."
...
- **Verified (scaling book, [jax-ml.github.io/scaling-book](https://jax-ml.github.io/scaling-book/)):** ICI arithmetic intensity C/W_ici ≈ 2,550 for v5p (4.59×10¹⁴ FLOPs/s over 2×9×10¹⁰ B/s per axis); the FSDP/DP compute-bound condition (per-chip batch > 2,550/M_X); the TP bound Y > M_Y·F/2,550; the mixed FSDP+TP batch floor α²/(M_X·M_Y·F); DCN intensity ≈ 73,440 and the ~73K-token per-pod DP floor at ~6.25 GB/s DCN per chip.
- **Derived (arithmetic from stated assumptions):** 9×10²⁴ FLOPs, two-pod/~25-day sizing; mesh factorization (2 × 35 × 4 × 64 = 17,920) and the 2,048 tokens/chip per-chip batch with its ~1.6× margin over the FSDP roofline; ~5.25 s step and 51% MFU decomposition; ~90 GB/chip/step FSDP volume and the ~17 GB/s rate; Young's-approximation checkpoint interval; the goodput waterfall (both bars) and the ~118,000 chip-day gap; DCN grad-reduce hiding margin (~250× over the DCN floor).
